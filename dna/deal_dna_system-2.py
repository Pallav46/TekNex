...
class DealDNAPredictor:
    def predict_single_deal(self, deal_data):
        """
        API Helper: Predicts outcome for a single partial deal (dictionary input).
        """
        if self.pass_centroids is None:
            raise ValueError("Model not trained! Call .train() first.")

        # 1. Convert Dictionary to DataFrame
        raw_df = pd.DataFrame([deal_data])
        
        # 2. Process Data (Clean & Calculate)
        processed_df = self._clean_input_data(raw_df.copy())
        row = processed_df.iloc[0]
        
        # 3. Create DataFrame for transformation
        row_df = pd.DataFrame([row])
        
        # Fill missing cols with NaN
        for col in self.numeric_features + self.categorical_features:
            if col not in row_df.columns:
                row_df[col] = np.nan
        
        # 4. Transform to Vector
    ...
    def save_model(self, filepath):
        """
        Saves the trained model components to a pickle file.
        """
        if self.pass_centroids is None:
            raise ValueError("Model has not been trained yet. Cannot save.")
        
        # We need to save the models AND the preprocessor (scaling/encoding logic)
        model_data = {
        ...
        print(f"[System] Model successfully saved to {filepath}")
    ...
    def load_model(cls, filepath):
        ...
        instance = cls(
            n_pass_clusters=model_data['n_pass_clusters'],
            n_fail_clusters=model_data['n_fail_clusters']
        )
        
        # Restore components
        instance.pass_kmeans = model_data['pass_kmeans']
        instance.fail_kmeans = model_data['fail_kmeans']
        instance.preprocessor = model_data['preprocessor']
        instance.pass_centroids = model_data['pass_centroids']
        instance.fail_centroids = model_data['fail_centroids']
