...
class SalesExecutiveMatcher:
    ...
    """
    def predict_new_customer(self, customer_data, top_k=5):
        """
        API Helper: Predicts matches for a raw dictionary (JSON) input
        without needing the customer to exist in the CSV file.
        """
        # 1. Create feature vector from the dictionary
        # The existing _create_customer_feature_vector works with dicts/Series
        customer_vector = self._create_customer_feature_vector(customer_data)
        
        # 2. Match to best cluster
        best_cluster = self._match_customer_to_cluster(customer_vector)
        
        # 3. Rank executives
        top_execs_ids = self._rank_executives_in_cluster(best_cluster, customer_data, top_k)
        
        # 4. (Optional) Fetch details for these IDs to return cleaner JSON
        results = []
        for sp_id in top_execs_ids:
            sp_row = self.salespeople_df[self.salespeople_df['SalespersonID'] == sp_id].iloc[0]
            results.append({
                "id": sp_id,
                "name": sp_row['Name'],
                "experience": float(sp_row['ExperienceYears']),
                "branch": sp_row.get('DealershipBranch', 'Main')
            })
            
        return results
    
    def __init__(self, customers_path='/Users/jaswindars/Desktop/Jassi/Project/Hackathon/sales executive/customers.csv', salespeople_path='/Users/jaswindars/Desktop/Jassi/Project/Hackathon/sales executive/salespeople.csv'):
        """
...
def main():
    ...
    matcher = SalesExecutiveMatcher(
        customers_path='/Users/jaswindars/Desktop/Jassi/Project/Hackathon/sales executive/customers.csv',
        salespeople_path='/Users/jaswindars/Desktop/Jassi/Project/Hackathon/sales executive/salespeople.csv'
    )
